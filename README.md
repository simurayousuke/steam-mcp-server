...
The server also exposes MCP resources for Steam Web API catalog coverage, interface lists, method lists, method schemas, app details, app news, app stats schema, player summaries, owned games, wishlists, followed games, playtime, app achievements, app stats, ban status, recently played games, Steam levels, badges, badge progress, friend lists, vanity-profile public wishlists, and `steam://me` authorized-user shortcuts including a combined overview.
