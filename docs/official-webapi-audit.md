...
| --- | --- | --- | --- |
| `IAuthenticationService` | Catalog discovery only. Steam OpenID and OAuth are the supported MCP login paths. | Password, QR, Steam Guard, mobile-confirmation, auth-session polling, and risk quiz methods such as `BeginAuthSessionViaCredentials`, `BeginAuthSessionViaQR`, `UpdateAuthSessionWithSteamGuardCode`, `UpdateAuthSessionWithMobileConfirmation`, `PollAuthSessionStatus`, and `NotifyRiskQuizResults`. | https://api.steampowered.com/ISteamWebAPIUtil/GetSupportedAPIList/v1/?format=json |
...
| `IClientStats_1046930` | Catalog discovery only. | `ReportEvent` is telemetry submission, not a read workflow. | https://api.steampowered.com/ISteamWebAPIUtil/GetSupportedAPIList/v1/?format=json |
...
| `IHelpRequestLogsService` | Catalog discovery only. | `UploadUserApplicationLog` uploads local log contents, and `GetApplicationLogDemand` is a POST-based support workflow that needs a separate privacy review before becoming a high-level tool. | https://api.steampowered.com/ISteamWebAPIUtil/GetSupportedAPIList/v1/?format=json |
...
| `IPlayerService` | Owned games, recently played games, single-game playtime, Steam level, badges, community badge progress, friends, followed games, and followed-game count tools. | `RecordOfflinePlaytime` and any other playtime mutation methods. | https://partner.steamgames.com/doc/webapi/IPlayerService |
...
| `ISteamBroadcast` | Catalog discovery only. | `ViewerHeartbeat` is a heartbeat side-effect, and `PlayerStats` is POST-based broadcast telemetry. | https://api.steampowered.com/ISteamWebAPIUtil/GetSupportedAPIList/v1/?format=json |
