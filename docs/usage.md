...
- Publisher-only tools, including app beta/build/depot/server metadata, partner app lists, banned-player records, Workshop finalized contributors, and user group list queries, require `STEAM_PUBLISHER_KEY`; they are read-only in this server and are kept separate from user Web API keys.
